<div class="widget">
<header><h2><img src="images/icon/chart_organisation.png"> ลำดับชั้นสายงาน</h2></header>
<section>
...
<ul id="linertree">
	<!-- upline begin -->
    <li><span class="upline <?=$uplinevip==1?"vipst":"norst";?>">(<?=$jeje['sm_code'];?>) <?=$jeje['sm_name'];?> || <? if($uplinevip==1){?>สมาชิกพรีเมียม<? }else{?>ผู้ดูแลระบบ<? }?> || <?=date("d F Y",strtotime(readname("sli_upd","system_liner","sli_sr_reply",$target)));?></span></li>
    <!-- upline end -->
	
    <!-- target begin -->
    <li><span class="target  <?=$requireply['sli_vip']==1?"vipst":"norst";?>">อัฟไลน์ (<?=$jojo['sm_code'];?>) <?=$jojo['sm_name'];?> || <? if($requireply['sli_vip']==1){?>สมาชิกพรีเมียม<? }else{?>สมาชิกปกติ<? }?> || <?=date("d F Y",strtotime($requireply['sli_upd']));?></span>
        
        <!-- line 1 begin -->
        <ul>
        <? $linel1=queryx2("select system_liner.sli_vip,system_liner.sli_upd,system_liner.sli_sr_reply,system_member.sm_code,system_member.sm_name from system_liner left join system_member on system_liner.sli_sr_reply=system_member.sm_id where system_liner.sli_sr_target='$reply' limit 3"); while($fl1=mysql_fetch_array($linel1)){ $l1id=$fl1['sli_sr_reply'];?>
            <li><span class="reply-l1 <?=$fl1['sli_vip']==1?"vipst":"norst";?>"><b>สมาชิกชั้นที่ 1</b> (<?=$fl1['sm_code'];?>) <?=$fl1['sm_name'];?> || <? if($fl1['sli_vip']==1){?>สมาชิกพรีเมียม<? }else{?>สมาชิกปกติ<? }?> || <?=date("d F Y",strtotime($fl1['sli_upd']));?></span>
